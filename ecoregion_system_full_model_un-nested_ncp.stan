...
transformed parameters {
  ...
  for (c in 1:num_covariates) {
    ...
    for (eco in 1:num_ecoregions){
      beta_eco[eco, c] = beta_eco_raw[eco, c] * level_2_sigma[c];
    }
  }
}
model {
  ...
  for (c in 1:num_covariates) {
    ...
    // }
    for (eco in 1:num_ecoregions){
      beta_eco[eco, c] ~ normal(0, level_2_sigma[c]);
      // beta_eco_raw[eco, c] ~ std_normal();
    }
  }
  if(run_estimation==1){
    for (n in 1:N) {
      y[n] ~ normal(x[n] * (beta_eco[eco_idx[n]] + beta_system[system_idx[n]]), observation_sigma);
    }
  }
}
generated quantities{
  vector[N] y_sim;
  for (n in 1:N) {
    y_sim[n] = normal_rng(x[n] * (beta_eco[eco_idx[n]] + beta_system[system_idx[n]]), observation_sigma);
  }
}
